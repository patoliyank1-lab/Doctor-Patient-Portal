...
export default function DoctorNotificationsPage() {
  ...
  return (
    <PageContainer
      title="Notifications"
      subtitle="Stay updated on appointments and account activity."
      action={
        unreadCount > 0 ? (
          <button type="button" disabled={markingAll} onClick={handleMarkAll}
    ...
    >
      {loading ? (
        ...
        <div className="space-y-2">
          {unreadCount > 0 && (
            <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-400">{unreadCount} unread</p>
          )}
          {notifications.map((n) => (
            <div key={n.id}
              className={`group flex items-start gap-4 rounded-2xl border p-4 transition-all ${
                n.isRead ? "border-slate-100 bg-white" : "border-blue-100 bg-blue-50/60 shadow-sm"
              }`}>
              ...
              <div className="flex shrink-0 items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                {!n.isRead && (
                  <button type="button" title="Mark as read" onClick={() => handleMarkRead(n.id)}
                ...
                <button type="button" title="Delete" disabled={deletingId === n.id} onClick={() => handleDelete(n.id)}
